 ...
 * This class has been slightly modified from the one given in class (get rid of the label)
**/
  ...
  public DataSet(String filename)
  {
    try
    {
      ...
      for (int k=0;k<size;k++)
      {
        ...
        //classLabel = Integer.parseInt(st.nextToken());      // don't need this
        for (int i=0;i<attributeNum;i++)
        {
          attributes[i] = (new Double(st.nextToken())).doubleValue();
        }
        examples[k] = new Example(0,attributes);
      }
    }
    catch (IOException e)
    {
